	...
	class FooGallery_Pro_Datasource_Folders {
		...
		function render_datasource_modal_content( $foogallery_id, $datasource_value ) {
			...
			<p>
				...
				<span class="foogallery-datasource-folder-selected"><?php echo empty( $folder ) ? esc_html__( 'nothing yet', 'foogallery' ) : $folder; ?></span>
		...
		function render_metadata( $attachments, $folder, $metadata_source ) {
			...
			if ( 'database' === $metadata_source ) {
				...
				if ( false !== $metadata_data ) {
					echo '<p>' . sprintf( esc_html__( 'Loading metadata from WordPress database option %s', 'foogallery' ), '<code>' . $option_key . '</code>' );
				...
				if ( $image_count > $image_metadata_count ) {
					...
					echo '<p><strong>' . sprintf( esc_html__( 'There are %d images with missing metadata!', 'foogallery' ), $image_count - $image_metadata_count ) . '</strong></p>';
				}
                ?>
				...
				if ( $folder_exists ) {
					...
					if ( $wp_filesystem->exists( $json_path ) ) {
						echo '<p>' . sprintf( esc_html__( 'Loading metadata from %s', 'foogallery' ), '<code>' . trailingslashit( $folder ) . $this->image_metadata_file() . '</code>' ) . '</p>';
					...
					if ( $metadata_file_exists ) {
						if ( $json_last_error_code !== JSON_ERROR_NONE ) {
							...
							echo '<p>' . sprintf( esc_html__( 'There was a problem reading metadata from %s. Please check that the file contains valid JSON. You can use a website like %s to help validate your JSON data.', 'foogallery' ), $this->image_metadata_file(), '<a href="https://jsonlint.com/" target="_blank">JSONLint</a>' ) . '</p>';
							echo '<p>' . esc_html__( 'Error : ', 'foogallery' ) . $json_last_error . '</p>';
						} else {
							if ( $image_count > $image_metadata_count ) {
								//there is missing metadata
								echo '<p><strong>' . sprintf( esc_html__( 'There are %d images with missing metadata!', 'foogallery' ), $image_count - $image_metadata_count ) . '</strong></p>';
						...
						echo '<p>' . sprintf( esc_html__( 'We read JSON metadata information about each image from the file (%s) which you need to save to the same folder.', 'foogallery' ), '<i>' . $this->image_metadata_file() . '</i>' );
						echo '<br />';
						echo '</p>';
					}
					...
					if ( $image_metadata_count === 0 ) {
						...
						echo '<p>' . sprintf( esc_html__( 'To use it: copy the JSON data below, change it to your liking, save it to a file named %s, and finally upload/transfer/FTP the file into the same folder on your server.', 'foogallery' ), $this->image_metadata_file() ) . '</p>';
		...
		function render_images( $attachments, $metadata_source ) {
			...
			if ( $image_count > 0 ) {
				echo '<p>' . sprintf( esc_html__( '%s images found in the folder.', 'foogallery' ), $image_count ) . '</p>';
