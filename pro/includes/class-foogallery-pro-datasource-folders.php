	...
	class FooGallery_Pro_Datasource_Folders {
		...
		function render_datasource_modal_content( $foogallery_id, $datasource_value ) {
			...
			<script type="text/javascript">
				document.foogalleryDatasourceFolderNonce = '<?php echo esc_js( wp_create_nonce( "foogallery_datasource_folder_change" ) ); ?>';
			</script>
			...
			<p>
				...
				<span class="foogallery-datasource-folder-selected"><?php echo empty( $folder ) ? esc_html__( 'nothing yet', 'foogallery' ) : esc_html( $folder ); ?></span>
		...
		function render_metadata( $attachments, $folder, $metadata_source ) {
			...
			if ( 'database' === $metadata_source ) {
				...
				if ( false !== $metadata_data ) {
					echo '<p>' . sprintf( esc_html__( 'Loading metadata from WordPress database option %s', 'foogallery' ), '<code>' . esc_html( $option_key ) . '</code>' );
					echo ' <a href="#" class="foogallery-server-image-metadata-clear">' . esc_html__( 'Clear Metadata', 'foogallery' ) . '</a>';
					echo '</p>';
				}

				if ( $image_count > $image_metadata_count ) {
					//there is missing metadata
					echo '<p><strong>' . sprintf( esc_html__( 'There are %d images with missing metadata!', 'foogallery' ), intval( $image_count ) - intval( $image_metadata_count ) ) . '</strong></p>';
				}
                ?>
				...
				if ( $folder_exists ) {
					...
					if ( $wp_filesystem->exists( $json_path ) ) {
						echo '<p>' . sprintf( esc_html__( 'Loading metadata from %s', 'foogallery' ), '<code>' . esc_html( trailingslashit( $folder ) . $this->image_metadata_file() ) . '</code>' ) . '</p>';
					...
					if ( $metadata_file_exists ) {
						if ( $json_last_error_code !== JSON_ERROR_NONE ) {
							echo '<p><strong>' . esc_html__( 'ERROR reading metadata file!', 'foogallery' ) . '</strong></p>';
							echo '<p>' . sprintf( esc_html__( 'There was a problem reading metadata from %s. Please check that the file contains valid JSON. You can use a website like %s to help validate your JSON data.', 'foogallery' ), esc_html( $this->image_metadata_file() ), '<a href="https://jsonlint.com/" target="_blank">JSONLint</a>' ) . '</p>';
							echo '<p>' . esc_html__( 'Error : ', 'foogallery' ) . esc_html( $json_last_error ) . '</p>';
						} else {
							if ( $image_count > $image_metadata_count ) {
								//there is missing metadata
								echo '<p><strong>' . sprintf( esc_html__( 'There are %d images with missing metadata!', 'foogallery' ), intval( $image_count - $image_metadata_count ) ) . '</strong></p>';
							} else {
								echo '<p><strong>' . esc_html__( 'Woohoo! There is no missing metadata!', 'foogallery' ) . '</strong></p>';
							}
						}
					} else {
						echo '<p><strong>' . esc_html__( 'NO metadata file found in folder!', 'foogallery' ) . '</strong></p>';
						echo '<p>' . sprintf( esc_html__( 'We read JSON metadata information about each image from the file (%s) which you need to save to the same folder.', 'foogallery' ), '<i>' . esc_html( $this->image_metadata_file() ) . '</i>' );
						echo '<br />';
						echo '</p>';
					}

					if ( $image_metadata_count === 0 ) {
						echo '<p>' . esc_html__( 'Below is the automatically generated JSON metadata for all images found in the folder.', 'foogallery' ) . '</p>';
						echo '<p>' . sprintf( esc_html__( 'To use it: copy the JSON data below, change it to your liking, save it to a file named %s, and finally upload/transfer/FTP the file into the same folder on your server.', 'foogallery' ), esc_html( $this->image_metadata_file() ) ) . '</p>';
					} else if ( $image_count > $image_metadata_count ) {
						echo '<p>' . esc_html__( 'Below is the automatically generated JSON metadata, merged together with the existing metadata read from file:', 'foogallery' ) . '</p>';
					} else {
						echo '<p>' . esc_html__( 'Below is the JSON metadata read from file:', 'foogallery' ) . '</p>';
					}
					...
					echo '<p>' . esc_html__( 'Please note : you can change the sort order of the images by changing the order of the metadata within the metadata file.', 'foogallery' ) . '</p>';
				}
			}
		}
		...
		function render_images( $attachments, $metadata_source ) {
			...
			if ( $image_count > 0 ) {
				echo '<p>' . sprintf( esc_html__( '%s images found in the folder.', 'foogallery' ), esc_html( $image_count ) ) . '</p>';
				...
				foreach ( $attachments as $attachment ) {
					...
					if ( $editable ) {
						echo '<a href="#" class="foogallery-server-image-list-edit" title="' . esc_html__( 'Edit the metadata for this image', 'foogallery' ) . '"><span class="dashicons dashicons-info"></span></a>';
					}
					echo '<span title="' . esc_html__( 'Missing Metadata!', 'foogallery' ) . '" class="missing dashicons dashicons-warning"></span>';
				...
				echo '<p>' . esc_html__( 'No images found in the folder.', 'foogallery' ) . '</p>';
			}
		}
		...
		function render_filesystem_tree( $folder = '/' ) {
			...
			if ( $folder !== '/' ) {
				...
				echo '<li><a title="' . esc_html__( 'Go up a level', 'foogallery' ) . '" href="#" data-folder="' . esc_attr( $up_folder ) . '"><i class="dashicons dashicons-category"></i>..</a></li>';
			}
