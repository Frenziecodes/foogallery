...
if ( ! class_exists( 'FooGallery_Pro_Datasource_Lightroom' ) ) {
    ...
    class FooGallery_Pro_Datasource_Lightroom {
		...
		function render_datasource_modal_content( $foogallery_id, $datasource_value ) {
            ...
            $wplr_link = '<a href="' . $wplr_url . '" target="_blank">' . esc_html__('WP/LR Sync', 'foogallery') . '</a>';
			...
			<?php } else { ?>
				<p><?php echo sprintf( esc_html__('You need to purchase the %s plugin in order to sync your Adobe Lightroom collections with your WordPress Media Library.', 'foogallery'), esc_html( $wplr_link ) ); ?></p>
				<p><?php echo esc_html__('WP/LR Sync is a Lightroom Publishing Service for WordPress. It exports your photos to WordPress, the folders and collections from Adobe Lightroom and keeps it all synchronized.', 'foogallery'); ?></p>
				<a href="<?php echo esc_url( $wplr_url ); ?>" target="_blank"><img src="https://store.meowapps.com/wp-content/uploads/2017/03/meow-apps.png" width="500" /></a>
			<?php } ?>
            <?php
		}
		...
		function get_collection_info() {
			if ( check_admin_referer( 'foogallery_datasource_lightroom_select', 'nonce' ) ) {
				...
				$media = $wplr->get_media_from_collection( $collectionId );
				echo sprintf( esc_html__( '%1$s contains %2$d images.', 'foogallery' ), esc_html( $collection->name ), count( $media ) );
			}
            ...
            <script type="text/javascript">
                ...
                jQuery(function ($) {
                    $('.foogallery-datasource-lightroom').on('click', 'button.remove', function (e) {
                        ...
                        $('#<?php echo esc_attr( FOOGALLERY_META_DATASOURCE_VALUE ); ?>').val('');

						//clear the datasource
						$('#<?php echo esc_attr( FOOGALLERY_META_DATASOURCE ); ?>').val('');
