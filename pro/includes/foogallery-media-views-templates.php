<script type="text/html" id="tmpl-fg-importer">
	...
	echo '<input type="hidden" class="fgi_nonce" value="' . esc_attr( $nonce ) . '"/>';
	?>
...
<?php
	$help = sprintf('<a href="#toggle-help">%s</a>', esc_html_e("help", "foogallery"));
?>
	...
	<div class="fgi-splash fgi-getting-started-instructions">
		...
		<p><small><?php printf(esc_html__("See the %s for more information.", "foogallery"), esc_html($help)); ?></small></p>
	</div>
		...
		<div class="fgi-form">
			...
			<label class="fgi-row">
				...
				<span class="fgi-col-input">
							...
							<span class="fgi-browse-inner">
								...
								<span class="fgi-browse-col-input">
									...
									<input type="text" name="urls[{{name}}]" value="{{url}}" spellcheck="false" data-type="{{name}}"
												 data-messages='{"required":"<?php esc_html_e("You must supply at least one URL.", "foogallery") ?>","pattern":"<?php printf(esc_html__("Please enter a %s file.", "foogallery"), "{{type.text}}") ?>"}'
												 data-pattern="(?:\/|=)(?<name>[^\/]+?)\.(?<ext>{{type.pattern}})(?:$|\?|&|#|,)" data-required="[name^='urls[']"/>
									<# } else { #>
									<input type="text" name="urls[{{name}}]" value="{{url}}" spellcheck="false" data-type="{{name}}"
												 data-messages='{"pattern":"<?php printf(esc_html__("Please enter a %s file.", "foogallery"), "{{type.text}}") ?>"}'
												 data-pattern="(?:\/|=)(?<name>[^\/]+?)\.(?<ext>{{type.pattern}})(?:$|\?|&|#|,)"/>
									<# } #>
								</span>
								<span class="fgi-browse-col-button">
									<button type="button" class="button button-secondary"
													data-options='{"type": {{type.mime}}, "title":"<?php printf(esc_html__("Select a %s video", "foogallery"), "{{type.text}}") ?>", "button":"<?php esc_html_e("Select Video", "foogallery") ?>"}'
									><?php esc_html_e("Select", "foogallery") ?></button>
								</span>
							</span>
						</span>
					...
					<span class="fgi-input-description">
						<?php printf(esc_html__("For more information on compatibility please see the following links: %s, %s and %s", "foogallery"), esc_url( $mp4 ), esc_url( $webm ) , esc_url( $ogg ) ) ?>
					</span>
				</span>
			</label>
		</div>
...
?>
	...
	<p>
		<?php printf(esc_html__("Displaying %s of %s results.", "foogallery"), esc_html( $offset ), esc_html( $total ) ) ?>
		<# if (data.total !== 1 && data.nextPage !== 0){ #>
			<span class="fgi-query-result-paged">
				...
				<span class="fgi-query-result-load-more"><?php printf(esc_html__("Would you like to %s?", "foogallery"), esc_url( $load_more ) ) ?></span>
				<span class="fgi-query-result-try-again"><?php printf(esc_html__("An error occurred loading additional results, %s?", "foogallery"), esc_url( $try_again ) ) ?></span>
			</span>
		<# } #>
	</p>
...
?>
	...
	<# if (data.total === 1){ #>
		<p><?php esc_html_e("Confirm the video is correct and then click the Import button.", "foogallery") ?></p>
	<# } else { #>
		<p><?php printf(esc_html__("Select the videos to import by clicking the thumbnails and then click the Import button, or would you like to %s?", "foogallery"), esc_html($import_album)); ?></p>
	<# } #>
...
?>
	...
	<# if (data.total === 1){ #>
		<p><?php esc_html_e("Confirm the video is correct and then click the Import button.", "foogallery") ?></p>
	<# } else { #>
		<p><?php printf(esc_html__("Select the videos to import by clicking the thumbnails and then click the Import button, or would you like to %s?", "foogallery"), esc_html( $import_channel) ) ?></p>
	<# } #>
...
?>
	...
	<# if (data.total === 1){ #>
		<p><?php esc_html_e("Confirm the video is correct and then click the Import button.", "foogallery") ?></p>
	<# } else { #>
		<p><?php printf(esc_html__("Select the videos to import by clicking the thumbnails and then click the Import button, or would you like to %s?", "foogallery"),esc_html( $import_playlist ) ) ?></p>
	<# } #>
...
?>
	...
	<# if (data.total === 1){ #>
		<p><?php esc_html_e("Confirm the video is correct and then click the Import button.", "foogallery") ?></p>
	<# } else { #>
		<p><?php printf(esc_html__("Select the videos to import by clicking the thumbnails and then click the Import button, or would you like to %s?", "foogallery"), esc_html($import_user)) ?></p>
	<# } #>
...
<script type="text/html" id="tmpl-fgi-youtube">
	...
	<?php
	$help = sprintf('<a href="#toggle-help">%s</a>', esc_html__("help", "foogallery"));
	$developers_console = sprintf('<a href="https://console.developers.google.com/" target="_blank">%s</a>', esc_html__("Google Developers Console", "foogallery"));
	?>

	<div class="fgi-splash fgi-youtube-api-key">
		<h2><?php esc_html_e("YouTube API key is required.", "foogallery") ?></h2>
		<p><?php printf(esc_html__("Create a new project in your %s to generate an api key.", "foogallery"), esc_html( $developers_console ) ) ?></p>
		<div class="button-hero-container">
...
<?php
	...
	$toggle_failed_message = sprintf(esc_html__("failed to import. Would you like to %s?", "foogallery"), $try_again_failed);
	...
	$toggle_cancelled_message = sprintf(esc_html__("were cancelled. Would you like to %s?", "foogallery"), $import_cancelled);
...
?>
	<div class="fgi-splash fgi-import-notification">
		...
		<# if (data.imported.length){ var imported = data.imported.length; #>
			<h2><?php printf(esc_html__("Successfully imported %s %s", "foogallery"), "{{imported}}", esc_html( $video_imported ) ) ?></h2>
			<# if (imported !== 1){ #>
				<p><?php printf(esc_html__("All imported videos have been added to the current %s selection.", "foogallery"), esc_html( $media_library ) ) ?></p>
			<# } else { #>
				<p><?php printf(esc_html__("The imported video has been added to the current %s selection.", "foogallery"), esc_html( $media_library ) ) ?></p>
			<# } #>
		<# } else { #>
		...
		<# if (data.failed.length){ var failed = data.failed.length; #>
			<p><?php echo esc_html($toggle_failed); ?></p>
		...
		<# if (data.cancelled.length){ var cancelled = data.cancelled.length; #>
			<p><?php echo esc_html( $toggle_cancelled ) ?></p>
