			...
			if ( foogallery_current_gallery_has_cached_value( 'filtering' ) ) {
				...
				if ( '' !== $filtering_type && isset( $filtering_options['position'] ) )  {
					...
					if ( $position === $filtering_position || 'both' === $filtering_position ) {
						echo '<nav id="' . esc_attr( $foogallery->container_id() ) . '_filtering-' . esc_attr( $position ) . '" class="fg-filtering-container fg-ph-' . esc_attr( $filtering_type ) . '"></nav>';
					}
				}
			}
		}
			    ...
			    if ( isset( $field['value'] ) ) {
			        ...
			        if ( isset( $levels ) ) {
			            ...
			            foreach ( $levels as $index => $level ) {
							...
							echo '<td>' . esc_html( $level['all'] ) . '</td>';
							echo '<td><code>' . esc_html( implode( ', ', $level['tags'] ) ) . '</code></td>';
                        }
			            echo '</tbody></table>';
                    }
                }
