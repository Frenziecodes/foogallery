...
if ( !function_exists( 'foo_check_php_version' ) ) {
	...
	function foo_check_php_version($plugin_title, $ver) {
		...
		if ( version_compare( $php_version, $ver ) < 0 ) {
			throw new Exception( esc_html( "$plugin_title requires at least version $ver of PHP. You are running an older version ($php_version). Please update!" ) );
...
if ( !function_exists( 'foo_check_wp_version' ) ) {
	...
	function foo_check_wp_version($plugin_title, $ver) {
		...
		if ( version_compare( $wp_version, $ver ) < 0 ) {
			throw new Exception( esc_html( "$plugin_title requires at least version $ver of WordPress. You are running an older version ($wp_version). Please update!" ) );
		}
	}	
}
