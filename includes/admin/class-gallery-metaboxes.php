	...
	class FooGallery_Admin_Gallery_MetaBoxes {
		...
		public function render_gallery_shortcode_metabox( $post ) {
			...
			<p class="foogallery-shortcode">
				<input type="text" id="foogallery_copy_shortcode" size="<?php echo esc_attr( strlen( $shortcode ) + 2 ); ?>" value="<?php echo esc_attr( $shortcode ); ?>" readonly="readonly" />
			</p>
			<p>
		...
		public function render_customcss_metabox( $post ) {
			...
			$example = '<code>#foogallery-gallery-' . esc_html( $post->ID ) . ' { }</code>';
			?>
			<p>
			<?php printf( esc_html__( 'Add any custom CSS to target this specific gallery. For example %s', 'foogallery' ), esc_html( $example ) ); ?>
			</p>
		...
		public function ajax_clear_gallery_thumb_cache() {
			if ( check_admin_referer( 'foogallery_clear_gallery_thumb_cache', 'foogallery_clear_gallery_thumb_cache_nonce' ) ) {
				...
				if ( $engine->has_local_cache() ) {
					...
					echo esc_html_e( 'The thumbnail cache has been cleared!', 'foogallery' );
				} else {
					echo esc_html_e( 'There was no thumbnail cache to clear.', 'foogallery' );
				}
			}
