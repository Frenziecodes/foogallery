		...
		function foogallery_settings() {
			...
			if ( $show_reset_message ) {
				...
				<div id="message" class="updated">
					<p><strong><?php printf( esc_html__( '%s settings reset to defaults.', 'foogallery' ), foogallery_plugin_name() ); ?></strong></p>
				</div>
			<?php } else if ( isset($_GET['settings-updated']) ) { ?>
				<div id="message" class="updated">
					<p><strong><?php printf( esc_html__( '%s settings updated.', 'foogallery' ), foogallery_plugin_name() ); ?></strong></p>
				</div>
			<?php }
		...
		function create_demo_galleries() {
			if ( check_admin_referer( 'foogallery_admin_import_demos' ) ) {
				...
				if ( $results === false ) {
					...
					echo sprintf(esc_html__('%d sample images imported, and %d demo galleries created!', 'foogallery'), $results['attachments'], $results['galleries']);
				}
			}
			die();
		}
