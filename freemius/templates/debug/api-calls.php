...
<h2><span>Total Time:</span><?php echo esc_html( Freemius_Debug_Bar_Panel::total_time() ); ?></h2>

<h2><span>Total Requests:</span><?php echo esc_html( Freemius_Debug_Bar_Panel::requests_count() ); ?></h2>
<?php foreach ( $counters as $method => $count ) : ?>
	<h2><span><?php echo esc_html( $method ); ?>:</span><?php echo esc_html( number_format( $count ) ); ?></h2>
<?php endforeach; ?>
<table class="widefat">
	<thead>
	<tr>
	...
	<?php foreach ( $logger as $log ) : ?>
		<tr>
			...
			<td><?php echo esc_html( number_format( 100 * $log['total'], 2 ) . ' ' . $ms_text ); ?></td>
			<td>
				<?php
				...
				<table class="widefat" style="display: none">
					...
					<?php for ( $i = 0, $bt = $log['backtrace'], $len = count( $bt ); $i < $len; $i ++ ) : ?>
						<tr>
							<td><?php echo esc_html( $len - $i ); ?></td>
							<td><?php if ( isset( $bt[ $i ]['function'] ) ) {
									echo esc_html( ( isset( $bt[ $i ]['class'] ) ? $bt[ $i ]['class'] . $bt[ $i ]['type'] : '' ) . $bt[ $i ]['function'] );
								} ?></td>
							<td><?php if ( isset( $bt[ $i ]['file'] ) ) {
									echo esc_html( substr( $bt[ $i ]['file'], $root_path_len ) . ':' . $bt[ $i ]['line'] );
								} ?></td>
						</tr>
					<?php endfor; ?>
					</tbody>
				...
				<td>
					<?php if ( 'GET' !== $log['method'] ) : ?>
						...
						printf(
							...
							esc_html( substr( $body, 0, 32 ) . ( 32 < strlen( $body ) ? '...' : '' ) )
						);
						if ( $pretty_print ) {
			...
			<td>
				<?php
					...
					if ( $is_not_empty_result ) {
						printf(
							'<a href="#" onclick="jQuery(this).parent().find(\'pre\').toggle(); return false;">%s</a>',
							esc_html( substr( $result, 0, 32 ) . ( 32 < strlen( $result ) ? '...' : '' ) )
						);
					}
			...
			<td><?php echo esc_html( number_format( 100 * ( $log['start'] - WP_FS__SCRIPT_START_TIME ), 2 ) . ' ' . $ms_text ); ?></td>
			<td><?php echo esc_html( number_format( 100 * ( $log['end'] - WP_FS__SCRIPT_START_TIME ), 2 ) . ' ' . $ms_text ); ?></td>
		</tr>
	<?php endforeach; ?>
	</tbody>
</table>
