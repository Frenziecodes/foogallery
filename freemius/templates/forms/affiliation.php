                        ...
                        <?php if ( $is_affiliate ) : ?>
                            <?php if ( $affiliate->is_active() ) : ?>
                                <div class="updated">
                                    <p><strong><?php
                                        echo sprintf(
                                            esc_html( fs_esc_html_inline( "Your affiliate application for %s has been accepted! Log in to your affiliate area at: %s.", 'affiliate-application-accepted', $slug ) ),
                                            esc_html( $plugin_title ),
                                            sprintf(
                                                '<a href="%s" target="_blank" rel="noopener">%s</a>',
                                                esc_url( $members_dashboard_login_url ),
                                                esc_html( $members_dashboard_login_url )
                                            )
                                        );
                                    ?></strong></p>
                                </div>
                    ...
                    <div class="entry-content">
                        <?php if ( ! $is_affiliate ) : ?>
                            <div id="application_messages_container">
                                <p><?php echo esc_html( sprintf( fs_text_inline( 'Like the %s? Become our ambassador and earn cash ;-)', 'become-an-ambassador', $slug ), esc_html( $module_type ) ) ); ?></p>
                                <p><?php echo esc_html( sprintf( fs_text_inline( 'Refer new customers to our %s and earn %s commission on each successful sale you refer!', 'refer-new-customers', $slug ), esc_html( $module_type ), esc_html( $commission ) ) ); ?></p>
                            </div>
                        <?php endif; ?>
                        <h3><?php fs_esc_html_echo_inline( 'Program Summary', 'program-summary', $slug ); ?></h3>
                        <ul>
                            <li><?php echo esc_html( sprintf( fs_text_inline( '%s commission when a customer purchases a new license.', 'commission-on-new-license-purchase', $slug ), esc_html( $commission ) ) ); ?></li>
                            <?php if ( is_object( $affiliate_terms ) && $affiliate_terms->has_renewals_commission() ) : ?>
                                <li><?php echo esc_html( fs_text_inline( 'Get commission for automated subscription renewals.', 'renewals-commission', $slug ) ); ?></li>
                            <?php endif; ?>
                            <?php if ( is_object( $affiliate_terms ) && ( ! $affiliate_terms->is_session_cookie() ) ) : ?>
                                <li><?php echo esc_html( sprintf( fs_text_inline( '%s tracking cookie after the first visit to maximize earnings potential.', 'affiliate-tracking', $slug ), esc_html( $affiliate_tracking ) ) ); ?></li>
                            <?php endif; ?>
                        ...
                        <div id="application_form_container" <?php echo ( $is_pending_affiliate ) ? '' : 'style="display: none"'; ?>>
                            ...
                            <form>
                                ...
                                <div class="input-container input-container-text">
                                    <label class="input-label"><?php echo esc_html( sprintf( fs_text_inline( 'Where are you going to promote the %s?', 'domain-field-label', $slug ), esc_html( $module_type ) ) ); ?></label>
                                    <input id="domain" type="text" value="<?php echo esc_attr( $domain ); ?>" class="domain regular-text" <?php echo esc_attr( $readonly ); ?>>
                                    <p class="description"><?php echo esc_html( sprintf( fs_text_inline( 'Enter the domain of your website or other websites from where you plan to promote the %s.', 'domain-field-desc', $slug ), esc_html( $module_type ) ) ); ?></p>
                                    <?php if ( ! $is_affiliate ) : ?>
                                ...
                                <div class="input-container input-container-text">
                                    ...
                                    <textarea id="statistics_information" rows="5" <?php echo esc_attr( $readonly ); ?> class="regular-text"><?php echo esc_textarea( $statistics_information ); ?></textarea>
                                    <?php if ( ! $is_affiliate ) : ?>
                                ...
                                <div class="input-container input-container-text">
                                    ...
                                    <textarea id="promotion_method_description" rows="5" <?php echo esc_attr( $readonly ); ?> class="regular-text"><?php echo esc_textarea( $promotion_method_description ); ?></textarea>
                                    <?php if ( ! $is_affiliate ) : ?>
                                        <p class="description"><?php echo esc_html( sprintf( fs_text_inline( 'Please provide details on how you intend to promote %s (please be as specific as possible).', 'promotion-method-desc-field-desc', $slug ), esc_html( $plugin_title ) ) ); ?></p>
                                    <?php endif; ?>
                                </div>
                                <?php if ( ! $is_affiliate ) : ?>
                                    <div>
                                        <input type="checkbox" id="legal_consent_checkbox">
                                        <label for="legal_consent_checkbox">I agree to the <a href="<?php echo esc_url( $affiliate_program_terms_url ); ?>" target="_blank" rel="noopener">Referrer Program</a>'s terms & conditions.</label>
                                    </div>
                                <?php endif; ?>
                            </form>
                        </div>
    ...
    <script type="text/javascript">
        jQuery(function ($) {
            ...
            $submitButton.click(function (evt) {
                ...
                $.ajax({
                    url       : <?php echo json_encode(Freemius::ajax_url()) ?>,
                    method    : 'POST',
                    data      : {
                        action   : '<?php echo json_encode($fs->get_ajax_action('submit_affiliate_application')) ?>',
                        security : '<?php echo json_encode($fs->get_ajax_security('submit_affiliate_application')) ?>',
                        module_id: '<?php echo json_encode($module_id) ?>',
                        affiliate: affiliate
                    },
                    beforeSend: function () {
