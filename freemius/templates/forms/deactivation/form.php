...
(function ($) {
	var reasonsHtml                    = <?php echo json_encode( $reasons_list_items_html ) ?>,
	    modalHtml                      =
        '<div class="fs-modal fs-modal-deactivation-feedback' . ( empty( $confirmation_message ) ? ' no-confirmation-message' : '' ) . '">'
        . ' <div class="fs-modal-dialog">'
        . '     <div class="fs-modal-header">'
        . '         <h4>' . esc_html( fs_esc_attr_echo_inline( 'Quick Feedback', 'quick-feedback' , $slug ) ) . '</h4>'
        . '     </div>'
        . '     <div class="fs-modal-body">'
        . '         <div class="fs-modal-panel" data-panel-id="confirm"><p>' . esc_html( $confirmation_message ) . '</p></div>'
        . '         <div class="fs-modal-panel active" data-panel-id="reasons"><h3><strong>' . esc_js( sprintf( fs_text_inline( 'If you have a moment, please let us know why you are %s', 'deactivation-share-reason' , $slug ), ( $fs->is_plugin() ? fs_text_inline( 'deactivating', 'deactivating', $slug ) : fs_text_inline( 'switching', 'switching', $slug ) ) ) ) . ':</strong></h3><ul id="reasons-list">' . esc_html( $reasonsHtml ) . '</ul></div>'
        . '     </div>'
        . '     <div class="fs-modal-footer">'
        . '         ' . esc_html( $anonymous_feedback_checkbox_html ) . ''
        . '         <label style="display: none" class="feedback-from-snooze-label"><input type="checkbox" class="feedback-from-snooze-checkbox"> <span>' . esc_js( fs_esc_js_echo_inline( 'Snooze this panel during troubleshooting', 'snooze-panel-during-troubleshooting', $slug ) ) . '</span><span style="display: none">' . esc_js( fs_esc_js_echo_inline( 'Snooze this panel for', 'snooze-panel-for', $slug ) ) . ' ' . esc_html( $snooze_select_html ) . '</span></label>'
        . '         <a href="#" class="button button-secondary button-deactivate"></a>'
        . '         <a href="#" class="button button-secondary button-close">' . esc_js( fs_esc_js_echo_inline( 'Cancel', 'cancel', $slug ) ) . '</a>'
        . '     </div>'
        . ' </div>'
        . '</div>',
	    $modal                         = $(modalHtml),
        ...
        otherReasonID                  = <?php echo (int) Freemius::REASON_OTHER; ?>,
        dontShareDataReasonID          = <?php echo (int) Freemius::REASON_DONT_LIKE_TO_SHARE_MY_INFORMATION; ?>,
	...
	function registerEventHandlers() {
		...
		$modal.on('click', '.fs-modal-footer .button', function (evt) {
			...
			if (_this.hasClass('allow-deactivate')) {
                ...
                var snoozePeriod = 0,
                    ...
                    if (shouldSnooze && <?php echo esc_js(Freemius::REASON_TEMPORARY_DEACTIVATION); ?> == selectedReasonID) {
		...
		$modal.on('click', 'input[type="radio"]', function () {
            ...
            $anonymousFeedback.toggle( <?php echo esc_js(Freemius::REASON_TEMPORARY_DEACTIVATION); ?> != selectedReasonID );
            $feedbackSnooze.toggle( <?php echo esc_js(Freemius::REASON_TEMPORARY_DEACTIVATION); ?> == selectedReasonID );

            if ( <?php echo esc_js(Freemius::REASON_TEMPORARY_DEACTIVATION); ?> == selectedReasonID ) {
	...
	function changeDeactivateButtonText() {
        ...
        var
            ...
            if (0 === $userReason.val().trim().length) {
                ...
                $deactivateButton.html('<?php echo esc_js(
                    $fs->is_plugin() ?
                        $deactivate_text :
                        sprintf($activate_x_text, $theme_text)
                ); ?>');
