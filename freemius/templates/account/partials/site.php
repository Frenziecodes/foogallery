...
<tr class="fs-site-details" data-blog-id="<?php echo esc_attr( $blog_id ); ?>"<?php if ( $is_registered ) : ?> data-install-id="<?php echo esc_attr( $install->id ); ?>"<?php endif; ?>>
    ...
    <!-- Site URL -->
    <td class="fs-field-url fs-main-column"><?php echo esc_html( fs_strip_url_protocol( $site['url'] ) ); ?></td>
    <!--/ Site URL -->
    ...
    <td><?php if ( $is_registered ) : ?>
            ...
            ?>
            <code><?php echo esc_html( $plan_title ); ?></code>
        <?php endif; ?></td>
    <!--/ Plan -->

    <!-- More details button -->
    <td><?php if ( $is_registered ) : ?>
            <button class="fs-show-install-details button button-small"><?php esc_html_e( 'More details', 'plugin-slug' ); ?> <i
        ...
        <td colspan="5">
            <table class="widefat fs-key-value-table">
                    ...
                    <td><?php if ( ! FS_Plugin_License::is_valid_id( $install->license_id ) ) : ?>
                        ...
                        <form action="<?php echo esc_url( $fs->_get_admin_page_url( 'account' ) ); ?>" method="POST">
                            ...
                            <button class="fs-toggle-tracking button button-small<?php if ( $is_disconnected ) {
                                echo ' button-primary';
                            } ?>" data-is-disconnected="<?php echo esc_attr( $is_disconnected ? 'true' : 'false' ); ?>">
                                <?php echo $is_disconnected ? esc_html__( 'Opt In', 'freemius' ) : esc_html__( 'Opt Out', 'freemius' ); ?>
                            </button>
                        </form>
                    <!--/ Toggle Usage Tracking -->
                    <?php endif; ?></td>
                ...
                <?php if ( $install->user_id != $main_user->id ) : ?>
                    ...
                    <?php if ( is_object( $user ) ) : ?>
                        ...
                        <tr <?php if ( 1 == $row_index % 2 ) {
                            ...
                            <td colspan="2"><code><?php echo esc_html( $user->get_name() ); ?></code></td>
                        </tr>
                        <?php $row_index++; ?>
                        <!--/ User Name -->
                        ...
                        <tr <?php if ( 1 == $row_index % 2 ) {
                            ...
                            <td colspan="2"><code><?php echo esc_html( $user->email ); ?></code></td>
                        </tr>
                        <?php $row_index++; ?>
                        <!--/ User Email -->
                        ...
                        <tr <?php if ( 1 == $row_index % 2 ) {
                            ...
                            <td colspan="2"><code><?php echo esc_html( $user->id ); ?></code></td>
                        </tr>
                        <?php $row_index++; ?>
                        <!--/ User ID -->
                    <?php endif; ?>
                ...
                <tr <?php if ( 1 == $row_index % 2 ) {
                    ...
                    <td><code><?php echo esc_html( $install->public_key ); ?></code></td>
                    <td></td>
                </tr>
                <?php $row_index++; ?>
                <!--/ Public Key -->
                ...
                <tr <?php if ( 1 == $row_index % 2 ) {
                    ...
                    <td>
                        <code><?php echo esc_html( FS_Plugin_License::mask_secret_key_for_html( $install->secret_key ) ); ?></code>
                        <?php if ( ! $is_whitelabeled ) : ?>
                        <input type="text" value="<?php echo esc_attr( $install->secret_key ); ?>"
                ...
                <?php if ( is_object( $license ) ) : ?>
                    ...
                    <tr <?php if ( 1 == $row_index % 2 ) {
                        ...
                        <td>
                            <code><?php echo esc_html( $license->get_html_escaped_masked_secret_key() ); ?></code>
                            <?php if ( ! $is_whitelabeled ) : ?>
                            <input type="text" value="<?php echo esc_attr( $license->secret_key ); ?>"
                                style="display: none" readonly/></td>
                            <?php endif; ?>
                        <?php if ( ! $is_whitelabeled ) : ?>
                        <td>
                    ...
                    <?php if ( ! is_object( $main_license ) || $main_license->id != $license->id ) : ?>
                        ...
                        <?php if ( ! $license->is_lifetime() && is_object( $subscription ) ) : ?>
                            ...
                            <tr <?php if ( 1 == $row_index % 2 ) {
                                ...
                                ?>
                                <td>
                                    <code><?php echo esc_html( $subscription->id ); ?> - <?php
                                        echo esc_html( 12 == $subscription->billing_cycle ?
                                ...
                                <?php
                                    ...
                                    $downgrade_confirmation_message    = sprintf(
                                        esc_js( $downgrade_x_confirm_text ),
                                        ( $fs->is_only_premium() ? esc_js( $cancelling_subscription_text ) : esc_js( $downgrading_plan_text ) ),
                                        esc_js( $plan->title ),
                                        esc_js( $human_readable_license_expiration )
                                    );

                                    $after_downgrade_message = ! $license->is_block_features ?
                                        sprintf( esc_js( $after_downgrade_non_blocking_text ), esc_js( $plan->title ), esc_js( $fs->get_module_label( true ) ) ) :
                                        sprintf( esc_js( $after_downgrade_blocking_text ), esc_js( $plan->title ) );
                                ?>
                                    ...
                                    <form id="fs_downgrade" action="<?php echo esc_url( $fs->_get_admin_page_url( 'account' ) ); ?>" method="POST">
                                        ...
                                        <?php wp_nonce_field( trim( "{$action}:{$blog_id}", ':' ) ); ?>
                                        <input type="hidden" name="blog_id" value="<?php echo esc_attr( $blog_id ); ?>">
                                        <button class="button button-small" onclick="if (confirm('<?php echo esc_js( $downgrade_confirmation_message . ' ' . $after_downgrade_message . ' ' . esc_js( $prices_increase_text ) ) ?>')) { this.parentNode.submit(); } else { return false; }"><?php echo esc_html( $downgrade_text ); ?></button>
                                    </form>
                                <?php endif; ?></td>
                            </tr>
                            <?php $row_index++; ?>
